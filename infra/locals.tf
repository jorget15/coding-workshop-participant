locals {
  ...
  python_names = {
    for name in local.python_dirs : name => {
      ...
      pip_requirements = true      # Shared layer: bundled into every Python Lambda so `from shared.x import ...` resolves
      shared_path      = abspath(format("%s/../backend/shared", path.module))    }
  }
